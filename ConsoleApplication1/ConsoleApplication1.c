...
#define printf my_printf
	...
	while (!exitProgram)
	{
		...
		{ // loops if input is not 1,2,3 or 4
		...
		else if (options == 2) // option to modify chart
		{
			...
			{ // loops if input is not 1,2,3,4,5,6,7 or 8
		...
		{ // option to use natural langauge to modify the chart
			char instruction[1024];
			char message[4096];
			const char *longString =
...
void changeCategoryValue(Category categories[], Scaled values[], int *numCategories, char *title, char *xAxisLabel, int *sortOption)
{
	...
	for (int i = 0; i < *numCategories; i++)
	{
		printf("%d. %s\n", i + 1, categories[i].name); // List out the list of existing catgeory options available in the chart
	}
	int choice = 0;
	while (1)
	{
	...
	{ // New value inserted must be an integer that is not 0
		printf("Invalid input. Please enter a numeric value: ");
	}

	scaleValues(categories, values, *numCategories); // Values are rescaled
	...
	sortCategories(categories, values, *numCategories, *sortOption); // Chart sorted base on the category option chosen previously
